...
# ====================== 数据读取 ======================
# data = remove_direct_wave(data, center_trace=14, velocity=1900, dt=0.00025, mute_time0=0.025, dx=2, taper=0, plot=False) #实际数据去直达波
# data = remove_direct_wave(data1, center_trace=24, velocity=1200, dt=0.00025, mute_time0=0.027, dx=2, taper=0, plot=False) #模拟数据去直达波


dt = 0.25  # 采样间隔(ms)

# clean = read_segy(r"D:\桌面\面波压制\训练数据\cs+GathEP.sgy", shotnum=260)[0][0:1200,:]
# clean = remove_direct_wave(clean, center_trace=24, velocity=1200, dt=0.00025, mute_time0=0.027, dx=2, taper=0, plot=False)
# clean[:, 24] = 1/2 * (clean[:, 23] + clean[:, 25])  # 修正中心道

# noise = read_segy(r"D:\桌面\面波压制\训练数据\cs_ground_roll.sgy", shotnum=260)[0][0:1200,:]
# noise = remove_direct_wave(noise, center_trace=24, velocity=1200, dt=0.00025, mute_time0=0.027, dx=2, taper=0, plot=False)

# clean, noise, noised, k = scale_to_neg1_1(clean, noise)

# np.save(r"D:\桌面\面波压制\Synthetic data test 2\合成含噪记录制作\clean.npy", clean)
# np.save(r"D:\桌面\面波压制\Synthetic data test 2\合成含噪记录制作\noise.npy", noise)
# np.save(r"D:\桌面\面波压制\Synthetic data test 2\合成含噪记录制作\noised.npy", noised)

# data = noised
# print(np.max(data), np.min(data))

# clean = np.load(r"D:\桌面\面波压制\Synthetic data test 2\合成含噪记录制作\clean.npy")
# result = np.load(r"D:\桌面\面波压制\Synthetic data test 2\DMSSL\denoised_shot_0.npy")
# data = result - clean

# clean = np.load(r"D:\桌面\面波压制\Synthetic data test 2\合成含噪记录制作\clean.npy")[100:500,5:43]
# result = np.load(r"D:\桌面\面波压制\Synthetic data test 2\F-K压制结果\result.npy")[100:500,5:43]
# data = result - clean
# data = normalize_traces_global(data)

# data = np.load(r"D:\桌面\面波压制\Synthetic data test 2\DMSSL\input_shot_0.npy")[100:500,5:43]

# data = np.load(r"D:\桌面\面波压制\Field data shot65\DMSSL\noise_0.npy")[0:1200,0:30]
# data = remove_direct_wave(data, center_trace=29, velocity=1900, dt=0.00025, mute_time0=0.025, dx=2, taper=0, plot=False)

# data = read_segy(r"D:\桌面\面波压制\训练数据\modified_SN03.sgy", shotnum=168)[65][0:1200,0:30]

# data = np.load(r"D:\桌面\面波压制\Field data shot65\DMSSL\input_shot_0.npy")[0:1200,0:30]
# data = remove_direct_wave(data, center_trace=29, velocity=1900, dt=0.00025, mute_time0=0.025, dx=2, taper=0, plot=False)

data = np.load(r"Synthetic data test 2\DMSSL\sw_mask_shot_0.npy")[100:500,5:43]
save_path1 = r"Synthetic data test 2\图片汇总"
# data = normalize_traces_global(data)
# np.save(r"Synthetic data test 2\npy文件汇总\input", data)

# ====================== Figure 1 ======================
n_samples, n_traces = data.shape
print(f"数据维度: {n_samples} 个采样点, {n_traces} 道")
time_axis = np.arange(n_samples) * dt + 0.025  # 时间轴，单位 ms，起始时间为 25ms
