...
class SeismicEnergyAnalyzer:
    ...
    def create_trace_mask(self, seismic_trace, dt, threshold=0.001):
        n_samples = len(seismic_trace)
        # t = np.arange(n_samples) * dt # 仅用于计算，可省略
...
class SeismicMaskDataset(Dataset):
    ...
    def generate_mask(self, shot_idx):
        ...
        # 【新增】根据参数决定损失计算区域
        if self.loss_on_full_non_sw:
            loss_region = non_sw_region  # 全部非面波区域
        else:
            loss_region = non_sw_mask_positions  # 仅掩码位置
    ...
    def visualize_sample(self, idx, figsize=(18, 10)):
        ...
        info_text = (f'样本 {idx}{flip_status} | 面波区域: {sw_ratio:.2f}% | '
...
if __name__ == "__main__":
    ...
    # 假设 SEGY 文件路径
    # 请注意：要运行此代码，您需要将 'segy_path' 更改为有效的 SEGY 文件路径。
    segy_file_path = r"D:\桌面\面波压制\data\dataset1_260.sgy"
