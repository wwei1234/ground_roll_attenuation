...
if __name__ == "__main__":
    ...
    # ========== 配置参数 ==========
    model_path = r"model\model_syn_1113\model_epoch_500.pth"
    input_segy_path = r"D:\桌面\面波压制\训练数据\dataset1_260.sgy"
    output_dir = r"D:\桌面\面波压制\合成记录处理结果\U-Net"
    os.makedirs(output_dir, exist_ok=True)

    USE_MASK_FOR_SURFACE_WAVE = True  # 是否在输入时替换面波为掩码
    BOOST_SURFACE_WAVE_ENERGY = True   # 是否进行面波能量增强
    ENERGY_BOOST_FACTOR = 1          # 手动指定能量增强系数（建议1.0-3.0）
    APPLY_VELOCITY_CONSTRAINT = False   # 是否应用速度约束
    SW_MIN_VELOCITY = 10              # 面波最小速度 (m/s)
    SW_MAX_VELOCITY = 160              # 面波最大速度 (m/s)
    norm = True                        # 是否进行逐道归一化
    
    target_shot_idx = 0            # 炮号
    shot_center = 24                # 炮点位置
    shotnum = 260                   # 炮集数量
